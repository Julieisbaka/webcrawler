...
from .exceptions import ConfigurationError, CrawlerError, RequestError
from .utils import LinkExtractor, RobotsTxtParser, URLValidator
...
class WebCrawler:
    ...
    def __init__(
        ...
        self.logger.info(
            f"WebCrawler initialized with anti-detection: {enable_anti_detection}"
        )
        self.logger.info(
            f"Features enabled: UA rotation={enable_user_agent_rotation}, "
    ...
    def _setup_anti_detection_components(
        ...
        if self.anti_detection_config.enable_user_agent_rotation:
            self.user_agent_rotator = UserAgentRotator(
                user_agents=custom_user_agents, random_rotation=random_rotation
            )
        else:
        ...
        if self.anti_detection_config.enable_proxy_rotation and proxy_list:
            ...
            self.logger.info(
                f"Proxy rotator initialized with {len(proxy_list)} proxies"
            )
        else:
        ...
        self.delay_manager = DelayManager(
            base_delay=self.delay, strategy=self.anti_detection_config.delay_strategy
        )
    ...
    def _validate_config(self):
        ...
        if not (
            self.seed_url.startswith("http://") or self.seed_url.startswith("https://")
        ):
    ...
    def _get_current_session(self) -> requests.Session:
        ...
        if (
            ...
            % self.anti_detection_config.session_rotation_interval
            == 0
        ):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "text/html" in content_type:
                    ...
                    page_data["title"] = self.link_extractor.extract_title(
                        response.text
                    )
                    page_data["meta_description"] = (
                        self.link_extractor.extract_meta_description(response.text)
                    )
                    page_data["links"] = self.link_extractor.extract_links(
                        response.text, url
                    )

                    self.logger.debug(
                        f"Extracted {len(page_data['links'])} links from {url}"
                    )
                else:
    ...
    def crawl(self) -> List[Dict[str, Any]]:
        ...
        self.logger.info(
            f"Configuration: depth={self.max_depth}, pages={self.max_pages}"
        )
        self.logger.info(
            f"Anti-detection: UA rotation={bool(self.user_agent_rotator)}, "
        ...
        if self.proxy_rotator:
            self.logger.info(
                f"Proxy pool: {len(self.proxy_rotator.proxies)} proxies available"
            )
        ...
        try:
            while self.url_queue and pages_crawled < self.max_pages:
                ...
                if page_data["error"]:
                    ...
                    self.logger.warning(
                        f"Failed to crawl {current_url}: {page_data['error']}"
                    )
                    ...
                    if depth < self.max_depth and page_data["links"]:
                        ...
                        self.logger.debug(
                            f"Added {new_links_added} new URLs to queue from {current_url}"
                        )

                # Log progress periodically
                if pages_crawled % 10 == 0:
                    self.logger.info(
                        f"Progress: {pages_crawled}/{self.max_pages} pages crawled, "
        ...
        self.logger.info(
            f"Crawling completed: {pages_crawled} pages crawled, "
    ...
    def get_anti_detection_stats(self) -> Dict[str, Any]:
        ...
        if self.user_agent_rotator:
            stats["user_agents"] = {
                ...
                "rotation_strategy": (
                    "random"
                    if self.user_agent_rotator.random_rotation
                    else "sequential"
                ),
            }
        ...
        if self.proxy_rotator:
            stats["proxy_stats"] = {
                ...
                "success_rate": (
                    (
                        len(self.proxy_rotator.proxies)
                        - len(self.proxy_rotator._failed_proxies)
                    )
                    / len(self.proxy_rotator.proxies)
        ...
        response_times = [
            p.get("response_time") for p in self.crawled_data if p.get("response_time")
        ]
        if response_times:
    ...
    def update_delay_strategy(
        self, strategy: str, base_delay: Optional[float] = None
    ) -> None:
        """
    ...
    def get_proxy_health(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "health_percentage": (
                ((total_proxies - failed_proxies) / total_proxies * 100)
                if total_proxies > 0
                else 0
            ),
        }
    ...
    def save_results(self, filename: str = "crawl_results.json", indent: int = 2):
        ...
        try:
            ...
            if not full_path.startswith(base_dir):
                raise CrawlerError(
                    f"Invalid output filename: {filename} (path traversal is not allowed)"
                )
